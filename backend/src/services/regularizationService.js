...
class RegularizationService {
    async applyRegularization(user, data) {
    ...
    async updateStatus(id, user, status) {
        ...
        const request = await db('attendance_regularizations').where({ id }).first();
        ...
        await db('attendance_regularizations')
            .where({ id })
            .update({
                status,
                approved_by: user.id,
